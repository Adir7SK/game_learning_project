...
class GameDetailsData:
    ...
    def load_weapons(self, file=cs.weapons_csv):
    ...
    def load_shields(self, file=cs.shields_csv):
    ...
    def get_armor_data(self, shield_file=cs.shields_csv, weapon_file=cs.weapons_csv):
    ...
    def generate_data(self):
        i = 0
        weight_options = [j*0.01 for j in range(1, fa.possible_weight)]
        density_options = [j*0.01 for j in range(1, fa.possible_densities)]
        weapon_data_collection = WeaponCollection(name="Hand", weight=1, material=cs.possible_materials[0], density=1, shape=cs.medium_broad + " " + cs.medium_length, efficient=False, sound="hhwee")
        for material in cs.possible_materials:
            for s in cs.possible_shapes.keys():
                ...
                i += 1
        self.write_tree(weapon_data_collection, cs.weapons_csv)
        ...
        for material in cs.possible_materials:
            for s in cs.possible_shapes.keys():
                ...
                shield_data_collection.insert_shield(name=name, weight=weight, material=material, density=density,
                                                     shape=shape, efficient=False, sound=sound)
                i += 1
        self.write_tree(shield_data_collection, cs.shields_csv)
